...
# @db_structure.query("SELECT TOP 20 * FROM tbl_Structure2;")
@db_structure.query("SELECT * FROM tbl_Structure2;")

# Iterator to all rows and search title project - field "id_parent" [1] = NULL
pbar = ProgressBar.new("Prepare data", @db_structure.data.size)
@project = Hash::new
@db_structure.data.each_index do |index_row|
	if @project.empty? and @db_structure.data[index_row][1] == -1
		@project["name"] = @db_structure.data[index_row][2].slice(0, 200)
		@project["short_name"] = @project["name"].slice(0, 40)
		@project["id"] = @db_structure.data[index_row][0]		
	else
		@db_structure.data[index_row][10] = Hash[ "level" => (@db_structure.data[index_row][3].split('\\').size-1), "size_route" => @db_structure.data[index_row][4].split('-').size ] 
	end	
...
def create_wbs(index_row)
	...
	VALUES (@id_projwbs, " + @project["id_project"].to_s + ", 565, 10, 1.00, 'N', 'WS_Open', '" + @code_wbs.next! + "', '" + @db_structure.data[index_row][2].slice(0, 300) + "', " + @id_last_wbs[level].to_s + ", 6, 0.88, 'Month', 'EV_Cmp_pct', 'EE_Rem_hr', @GUID_projwbs)"
	@db_pmdb.exec(str_query)	
end

def create_task(index_row, route=nil, relationship_self=false)
	id_parent = @db_structure.data.select { |row| row[0] == @db_structure.data[index_row][1] }
	...
	if ( @db_structure.data[index_row][1] != @project["id"] or relationship_self ) and @db_structure.data[index_row][10].include?("id_task")
	...
	VALUES 
	(@id_task, " + @project["id_project"].to_s + ", " + @id_last_wbs[@db_structure.data[index_row][10]["level"]].to_s + ", 1408, 1.0, 'CP_Drtn', 'TT_Task', 'DT_FixedRate', 'RV_OK', 'TK_NotStart', '" + @code_task.next! + "', '" + @db_structure.data[index_row][2].slice(0, 300) + "', 168, 168, null, null, 'CS_ALAP', @GUID_task)"			
	@db_pmdb.exec(str_query)
	...
	VALUES 
	(598, " + @db_structure.data[index_row][10]["id_task"].to_s + ", " + @project["id_project"].to_s + ", '" + (route == nil ? @db_structure.data[index_row][4].to_s : route.to_s) + "')"
	@db_pmdb.exec(str_query)	
	# Routes_full (ID=601)
	str_query = "INSERT INTO [dbo].[UDFVALUE] 
	([udf_type_id], [fk_id], [proj_id], [udf_text])
	VALUES 
	(601, " + @db_structure.data[index_row][10]["id_task"].to_s + ", " + @project["id_project"].to_s + ", '" + @db_structure.data[index_row][4].to_s + "')"
	@db_pmdb.exec(str_query)	
	# Plots (ID=599)
	str_query = "INSERT INTO [tempPMDB].[dbo].[UDFVALUE] ([udf_type_id], [fk_id], [proj_id], [udf_text])
	VALUES (599, " + @db_structure.data[index_row][10]["id_task"].to_s + ", " + @project["id_project"].to_s + ", '" + @db_structure.data[index_row][5] + "')"
	@db_pmdb.exec(str_query)	
	# # Materials (ID=600)
	# str_query = "INSERT INTO [tempPMDB].[dbo].[UDFVALUE] ([udf_type_id], [fk_id], [proj_id], [udf_text])
	...
	@db_structure.data.each_index do |index_row|
		...
		if @db_structure.data[index_row][10] != nil
			...
			if @db_structure.data[index_row][10]["size_route"] > 1
				...
				@db_structure.data[index_row][4].reverse.split('-').each do |route|				
